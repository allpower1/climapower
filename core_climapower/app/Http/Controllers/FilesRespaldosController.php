...
class FilesRespaldosController extends Controller
{
    ...
    {
        $this->middleware('auth')->except(['publicidad','avatares','adjuntomultimedia','adjuntoarea','adjuntoequipo','adjuntoacercanosotros','adjuntofondofooter','adjuntosliderhome','adjuntotestimonio','adjuntoproyecto','adjuntopartners']);
    }

    public function adjuntoarea(Request $request, $path)
    {
        abort_if(!Storage::disk('adjuntoarea')->exists($path),404,"El archivo no se encuentra en el disco.");

        return Storage::disk('adjuntoarea')->response($path);
    }
    ...
    public function adjuntosliderhome(Request $request, $path)
    {
        abort_if(!Storage::disk('adjuntosliderhome')->exists($path),404,"El archivo no se encuentra en el disco.");

        return Storage::disk('adjuntosliderhome')->response($path);
    }

    public function adjuntotestimonio(Request $request, $path)
    {
        abort_if(!Storage::disk('adjuntotestimonio')->exists($path),404,"El archivo no se encuentra en el disco.");

        return Storage::disk('adjuntotestimonio')->response($path);
    }

    public function adjuntoproyecto(Request $request, $path)
    {
        abort_if(!Storage::disk('adjuntoproyecto')->exists($path),404,"El archivo no se encuentra en el disco.");

        return Storage::disk('adjuntoproyecto')->response($path);
    }

    public function adjuntopartners(Request $request, $path)
    {
        abort_if(!Storage::disk('adjuntopartners')->exists($path),404,"El archivo no se encuentra en el disco.");

        return Storage::disk('adjuntopartners')->response($path);
    }
